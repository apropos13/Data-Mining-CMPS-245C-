...
#load dataset
dataset= pd.read_csv("../../ProcessedData/cluster_1_pts.csv")
original_dataset=dataset #keep a copy because after clustering we want the title/desc of the clusters
#get rid off the dummy variables introduced in the eda, since they are not easiliy interpretable
#original_dataset=original_dataset.iloc[:, :8]

#drop non numerical values
dataset=dataset.drop(['title', 'description', 'location', 'category','currency'], axis=1)
...
def fancy_dendrogram(*args, **kwargs):
    ...
    if not kwargs.get('no_plot', False):
        plt.title('Hierarchical Clustering Dendrogram (truncated to the last 40 steps)', fontsize=16)
...
def kmeans(num_clusters, tsneX, tsneY, distance_metric, plot_sse=False):
	...
	for item,loc in zip(n_best, plot_locations):
		...
		plt.title('Clusters=%d \n Silhouette Score= %.4f' %(clusters[item], silhouette_list[item]), fontsize=14)
...
def dbscan(epsilon_list,tsneX, tsneY, distance_metric):
	...
	for item,loc,e in zip(n_best, plot_locations, epsilon_list):
		...
		plt.scatter(tsneX, tsneY, c=labels_all[item])
		try:
			plt.title('Epsilon= %.4f,  Clusters=%d \n Silhouette Score= %.4f' %(e,total_clusters[item], silhouette_list[item]), fontsize=16)
		except TypeError:
			plt.title('Epsilon= %.4f,  Clusters=%d \n Silhouette Score= %.4f' %(e,1, 0), fontsize=16)
...
def hierarchical(num_clusters, tsneX, tsneY, distance_metric, plot_dendro=False):
	...
	for item,loc in zip(n_best, plot_locations):
		...
		plt.title('Clusters=%d \n Silhouette Score= %.4f' %(clusters[item], silhouette_average[item]), fontsize=14)
	...
	for item,loc in zip(n_best, plot_locations):
		...
		plt.title('Clusters=%d \n Silhouette Score= %.4f' %(clusters[item], silhouette_ward[item]), fontsize=14)
...
def get_cluster_points(estimator):
	...
	return {i: original_dataset.iloc[ np.where(estimator.labels_ == i)[0] ]  for i in range(estimator.n_clusters)}


def main():
	metric='euclidean'
	t_x, t_y= reduce_dim(False)
	epsilon_list=np.arange(0.1,2.4,0.1)
	kmeans(15, t_x, t_y, metric, plot_sse=True)
	hierarchical(15, t_x, t_y, metric, plot_dendro=True)
	gaussian(15, t_x, t_y, metric)
	#dbscan(epsilon_list, t_x, t_y, metric)

	#best 
	#best_avg=AgglomerativeClustering(linkage='complete', n_clusters=2).fit(dataset)
	#pprint.pprint(get_cluster_points(best_avg)[1])
	#get_cluster_points(best_avg)[1].to_csv('../../ProcessedData/cluster_1_pts.csv', index=False)
	#get_cluster_points(best_avg)[0].to_csv('../../ProcessedData/cluster_2_pts.csv', index=False)
